...
func Page(site *model.SiteInfo, page *model.Page, buf *bytes.Buffer) {
	buf.Grow(1546)
	buf.WriteString(`
...
<article class="cm9 c9 p9 m12 block-input">
    `)
	if page.Mate["headimg"] {
		buf.WriteString(`
    <div class="headimg card">
        <img src="`)
		Goh.EscapeHTML(page.Mate["headimg"], buf)
		buf.WriteString(`" alt="`)
		Goh.EscapeHTML(page.Mate["title"], buf)
		buf.WriteString(`">
    </div>
    `)
	}
